<template>
  <div class="body">
    <div class="concent">
      <h1 class="title">请选择，注册公司所属国家</h1>
     ...
     <template v-if='data1.length!=0'>
      ...
      <el-dialog title="开通申请" :visible.sync="applymodel" width="40%">
        <div class="title">
          您选择申请{{applyname}}国家注册公司，请认真填写以下注册公司申请信息
        </div>
        <el-form
          :model="ruleForm"
          :rules="rules"
...
<script>
import { companyList, companyOrder, companyDetails, companyConfig, ossupload, companySubmit } from '@/api/api'
export default {
  data () {
    return {
      detailsobj: {},
  ...
  methods: {
    ...
    orderinit () {
      companyOrder({pageSize: this.pageSize, pageNum: this.pageNum}).then((res) => {
    ...
    parClick (orderId) {
      companyDetails({orderId: orderId}).then((res) => {
    ...
    modellingClick (item) {
      ...
      companyConfig({templateId: item.templateId}).then((res) => {
        console.log(res)
        if (res.data.code == '0') {
