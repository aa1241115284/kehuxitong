<template>
  <div class="body">
    <div class="concent">
      <h1 class="title">请选择，在以下网站运营商里开设商店</h1>
     ...
     <template v-if='data1.length!=0'>
      ...
      <el-dialog title="开通申请" :visible.sync="applymodel" width="40%">
        <div class="title">
          您选择开通{{ applyname }}支付，请认真填写以下支付开通申请信息
        </div>
        <el-form
          :model="ruleForm"
          :rules="rules"
...
<script>
import { paymentlist, paymentorderlist, paymentorderdetails, paymenttemplate, ossupload, paymentapply } from '@/api/api'
export default {
  data () {
    return {
      detailsobj: {},
  ...
  methods: {
    ...
    orderinit () {
      paymentorderlist({pageSize: this.pageSize, pageNum: this.pageNum}).then((res) => {
    ...
    parClick (orderId) {
      paymentorderdetails({orderId: orderId}).then((res) => {
        this.dialogVisible = true
        this.detailsobj = res.data.data
        this.detailsobj.List = this.detailsobj.formVOList
      ...
      this.$refs[formName].validate((valid) => {
        ...
        this.$confirm('是否提交建站申请？', '提示', {
          ...
          .then(() => {
            ...
            obj.formVOList = this.submList
            paymentapply(obj).then((res) => {
              if (res.data.code == '0') {
    ...
    modellingClick (item) {
      ...
      paymenttemplate({templateId: item.templateId}).then((res) => {
        console.log(res)
        if (res.data.code == '0') {
