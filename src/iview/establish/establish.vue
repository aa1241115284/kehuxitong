    ...
    <div class="concent">
     ...
     <template v-if='data1.length!=0'>
      ...
      <el-dialog title="开通申请" :visible.sync="applymodel" width="40%">
        <div class="title">
          您选择在{{ applyname }}进行商城建站，请认真填写以下建站申请信息
        </div>
        <el-form
          :model="ruleForm"
          :rules="rules"
...
<script>
import { websitelist, websiteorderlist, websitedetails, websitetemplate, ossupload, websiteapply } from '@/api/api'
export default {
  data () {
    return {
      detailsobj: {},
  ...
  methods: {
    ...
    orderinit () {
      websiteorderlist({pageSize: this.pageSize, pageNum: this.pageNum}).then((res) => {
    ...
    parClick (orderId) {
      websitedetails({orderId: orderId}).then((res) => {
    ...
    modellingClick (item) {
      ...
      websitetemplate({templateId: item.templateId}).then((res) => {
        console.log(res)
        if (res.data.code == '0') {
