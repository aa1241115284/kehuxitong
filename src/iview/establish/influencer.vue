    ...
    <div class="concent">
     ...
     <template v-if='data1.length!=0'>
      ...
      <el-dialog title="开通申请" :visible.sync="applymodel" width="40%">
        <div class="title">
          您选择申请{{applyname}}网红公司，请认真填写以下营销申请信息
        </div>
        <el-form
          :model="ruleForm"
          :rules="rules"
...
<script>
import { celebrityList, celebrityOrder, celebrityDetails, celebrityConfig, ossupload, celebritySubmit } from '@/api/api'
export default {
  data () {
    return {
      detailsobj: {},
  ...
  methods: {
    ...
    orderinit () {
      celebrityOrder({pageSize: this.pageSize, pageNum: this.pageNum}).then((res) => {
    ...
    parClick (orderId) {
      celebrityDetails({orderId: orderId}).then((res) => {
    ...
    modellingClick (item) {
      ...
      celebrityConfig({templateId: item.templateId}).then((res) => {
        console.log(res)
        if (res.data.code == '0') {
